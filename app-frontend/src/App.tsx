...
function App() {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      console.log("Bot reply",response.data.answer.candidates[0].content.parts[0].text)

      const botMessage: Message = {
        id: (Date.now() + 1).toString(),
        content: response.data.answer.candidates[0].content.parts[0].text || "Sorry, I couldn't find an answer.",
        sender: "bot",
      }
              ...
              <div key={message.id} className={`flex ${message.sender === "user" ? "justify-end" : "justify-start"}`}>
                <Card
                  className={`max-w-[80%] md:max-w-[70%] p-3 border-0 ${
                    message.sender === "user" ? "bg-[#333333] text-white" : "bg-[#111111] text-[#f1f1f1]"
                  }`}
                >
